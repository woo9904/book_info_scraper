...
#----------------------------------------------------------------------
#함수
def abe_search(book_name):
    global lines, headers, df
    rare_url="https://www.abebooks.com/servlet/SearchResults?kn="
    option_url="&sts=t&cm_sp=SearchF-_-topnav-_-Results&ds=20"
    url=rare_url+book_name+option_url
    ...
    #href=soup.find("div", attrs={"class":"result-detail col-xs-8"})
 
    book=[]
    book_name_get=soup.find("span", attrs={"data-cy":"listing-title"}).get_text()
    #print(book_name_get)
    
    book_writer=soup.find("p", attrs={"data-cy":"listing-author"}).get_text()
    #print(book_writer)
    book_pub=soup.find("span", attrs={"class":"opt-publisher"}).get_text().split(" &")[0]
    #print(book_pub)
    book_year=soup.find("span", attrs={"class":"opt-publish-date"}).get_text()
    #print(book_year)

    book_ISBN=book_name
    #데이터 프레임 리스트 만들기 (ROW)
    book=[book_name_get, book_writer, book_pub, book_year, book_ISBN, 0]
    df.append(book)
    print(book)
    print(book_name+"----완료----")
